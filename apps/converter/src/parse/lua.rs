...
pub fn parse_champion_entry(content: &str, champion: &str) -> Result<HashMap<String, LuaValue>> {
    ...
    let entry = root
        ...
        .or_else(|| {
            root.iter()
                .find(|(_, value)| champion_alias_matches(value, champion))
                .map(|(_, v)| v)
        })
...
fn champion_alias_matches(value: &LuaValue, champion: &str) -> bool {
    let LuaValue::Table(map) = value else {
        return false;
    };
    for key in ["apiname", "name", "fullname"] {
        if let Some(candidate) = map.get(key).and_then(lua_value_to_string) {
            if candidate.eq_ignore_ascii_case(champion) {
                return true;
            }
        }
    }
...
pub fn parse_champion_module(content: &str) -> Result<HashMap<String, HashMap<String, LuaValue>>> {
    let root = parse_module_root(content)?;
...
pub fn parse_item_data(content: &str, item: &str) -> Result<HashMap<String, LuaValue>> {
    ...
    let entry = root
        ...
        .or_else(|| {
            root.iter()
                .find(|(_, value)| item_alias_matches(value, item))
                .map(|(_, v)| v)
        })
...
fn item_alias_matches(value: &LuaValue, item: &str) -> bool {
    let LuaValue::Table(map) = value else {
        return false;
    };
...
mod tests {
  ...
  ["Tester"] = {
    ...
    #[test]
    fn parse_item_table_with_escaped_quote_string() {
        let lua = r#"return {
    ["Anathema's Chains"] = {
        ...
        let item = parse_item_data(lua, "Anathema's Chains").unwrap();
        assert_eq!(
            lua_value_to_string(item.get("caption").unwrap()).unwrap(),
            "\"She swore to dedicate her life to his destruction...\""
        );
        assert_eq!(
            lua_value_to_string(item.get("buy").unwrap()).unwrap(),
            "2500"
        );
    }

    #[test]
    fn parse_item_data_matches_formatname_alias() {
        let lua = r#"return {
  ["Arcane Sweeper (Trinket)"] = {
    ...
    ["tier"] = 1,
        ...
        let item = parse_item_data(lua, "Arcane Sweeper").unwrap();
        assert_eq!(lua_value_to_string(item.get("tier").unwrap()).unwrap(), "1");
    }
}
