...
pub fn extract_balanced_templates(input: &str) -> Result<Vec<TemplateSpan>, ConvertError> {
    ...
    if output
        .warnings
        .iter()
        .any(|w| w.message == WarningMessage::MissingEndTagRewinding)
    {
        if let Some(start) = first_unbalanced_template_start(input) {
            return Err(ConvertError::UnbalancedBraces {
                name: format!("start@{}", start),
            });
        }

        return extract_balanced_templates_fallback(input, &index);
    }
...
fn extract_balanced_templates_fallback(
    input: &str,
    index: &LineIndex,
) -> Result<Vec<TemplateSpan>, ConvertError> {
    let bytes = input.as_bytes();
    let mut spans = Vec::new();
