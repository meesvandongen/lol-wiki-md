...
#[derive(Debug, Serialize, Deserialize)]
pub struct TemplateValidationReport { pub issues: Vec<TemplateValidationIssue> }
...
pub fn validate_templates(wiki_root: &Path, precision: u8) -> Result<TemplateValidationReport> {
    ...
    for ent in entries.flatten() {
        ...
        let Ok(spans) = extract_balanced_templates(&text) else { continue; };
        if spans.is_empty() { continue; }
        for sp in spans {
            let body = &sp.raw[2..sp.raw.len()-2];
            let inv = parse_invocation(body);
            let ctx = ExpanderCtx { precision, vars: Default::default() };
            if !reg.has_name(&inv.name) {
                issues.push(TemplateValidationIssue { file: path.file_name().unwrap().to_string_lossy().into_owned(), line: sp.line, col: sp.col, name: inv.name.clone(), code: "E_UNKNOWN_TEMPLATE".into(), message: format!("template '{}' not supported", inv.name) });
                *unknown_counts.entry(inv.name.clone()).or_insert(0) += 1;
                continue;
            }
            // Try expand to catch malformed params
            if let Err(e) = reg.expand(&inv, &ctx) {
                issues.push(TemplateValidationIssue { file: path.file_name().unwrap().to_string_lossy().into_owned(), line: sp.line, col: sp.col, name: inv.name.clone(), code: format!("{e}"), message: format!("failed to expand: {e}") });
            }
        }
    }
