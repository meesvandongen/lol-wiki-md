...
#[derive(Debug, Clone)]
pub struct ConversionOutcome { pub entity: String, pub output: PathBuf }

/// Utility: ensure path exists else error variant.
pub fn require_exists(path: &Path, err: ConvertError) -> Result<()> { if path.exists() { Ok(()) } else { Err(err) } }
...
pub fn write_if_changed(path: &Path, content: &str) -> Result<()> {
    if let Ok(existing) = std::fs::read_to_string(path) { if existing == content { return Ok(()); } }
