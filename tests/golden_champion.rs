...
fn golden_minimal_champion_hash_stable() {
    ...
    std::fs::write(root.join("export_out").join("HashTester.txt"), "{{#vardefine:X|5}}Ability text {{#var:X}} {{ap|40}}") .unwrap();
    // Module ChampionData (flat encoded path)
    std::fs::write(root.join("export_out").join("Module%3AChampionData%2Fdata.txt"), "return { ['HashTester'] = { hp = 600, hpGrowth = 100 } }").unwrap();
    let out_dir = root.join("out");
    convert_champion(root, &out_dir, "HashTester").unwrap();
