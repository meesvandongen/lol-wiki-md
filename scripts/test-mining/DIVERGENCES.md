# Test-mining divergence report

Generated by mining the live wiki corpus (`export_out/`, 597 entities + 3,310
transcluded template/module pages) and differential-testing the converter
against the wiki's own rendered HTML (`api.php?action=parse`). Reproduce with
the scripts in this directory (see `README.md`).

Method: every snippet below was rendered on the wiki (cached under the
gitignored `validation_reports/test-mining/render_cache.json`) and expanded
through the converter (`expand_inline_templates`). Pure value/parser-function
templates are
context-independent, so their standalone wiki render IS authoritative ground
truth. Templates whose level count depends on champion context (`ap`/`pp` with
implicit level counts) are NOT reliably comparable standalone and are excluded.

## Corpus coverage (PASS)

The full real corpus converts cleanly with **zero hard errors**. Against the
complete wiki download (21,831 files, 8,416 categories, 19,352 source pages):

- champions: 172/172
- items: 594/594
- runes: 77/77

(The earlier targeted export — all champions/items/runes from the data modules
plus transclusions — was 170/305/76; the full download adds Wild Rift variants
and items outside the main `ItemData` module, all of which also convert clean.)

So the fail-fast template registry recognizes every template actually reached
on the convertible champion/item/rune surface. The gaps below are reached only
inside template *definition* bodies or are silent (wrong-but-not-erroring)
rather than hard failures — which is exactly the dangerous, invisible class
AGENTS.md flags.

## Confirmed divergences (converter vs. wiki ground truth)

These are context-independent and unambiguous. Each is encoded as an
`#[ignore]`d test in `tests/template_combinations.rs` (module
`known_divergences`) asserting the wiki-correct value, so they are runnable on
demand (`cargo test -- --ignored`) and will pass once the handler is fixed.

| # | Input | Wiki renders | Converter renders | Cause | GH |
|---|-------|--------------|-------------------|-------|----|
| 1 | `{{#switch:z\|a=Apple\|b=Banana\|Default}}` | `Default` | `` (empty) | `SwitchExpander` only honors `#default=`, not MediaWiki's bare-last-parameter default. Silent: emits empty where a default exists. | #13 |
| 2 | `{{#expr:10 mod 3}}`, `{{#expr:5 > 3}}`, `{{#expr:sqrt 16}}` | `1`, `1`, `4` | `E_MALFORMED_TEMPLATE` | `parse/expr.rs` implements only `+ - * / ^` and `round/floor/ceil/abs`; missing `mod`, comparison (`< > = <= >= <>`), boolean (`and or not`), `e` notation, `sqrt/trunc/ln/...`. Comparison appears on 3 corpus pages. | #14 |
| 3 | `{{lc:Hello}}`, `{{uc:hello}}`, `{{ucfirst:hello}}`, `{{lcfirst:Hello}}` | `hello`, `HELLO`, `Hello`, `hello` | `E_UNKNOWN_TEMPLATE` | string-casing magic words not implemented. `lcfirst:` on 9 pages, used in `tip`/`Tip data` bodies. | #15 |
| 4 | `{{#replace:hello world\|world\|there}}` | `hello there` | `E_UNKNOWN_TEMPLATE` | ParserFunctions `#replace:` not implemented. Appears in `tt` gold-efficiency bodies. | #16 |
| 5 | `{{cis\|Aatrox}}` | `Aatrox's` | `Aatrox` | `IconUnwrapExpander` claims the possessive variants (`cis`/`cais`/`iis`/`sis`/`nies`/`uis`) but only appends `'s` for an explicit `'s` positional, so the bare-name form silently drops the possessive. | #17 |
| 6 | `(24{{divided by}}n)` (Bloodsong) | `(24 ÷ n)` | `(24n)` | `Template:Divided by` is literally `&nbsp;&divide;&nbsp;` and ignores args; operands are adjacent wikitext. `DividedByExpander` joins (absent) args with ` / `, so the no-arg inline form drops the ÷ entirely → garbled output on a real item. | #19 |
| 7 | `{{fd\|500\|750\|1000}}`, `{{fd\|0.6666666\|2}}`, `{{fd\|1.30}}` | `500`, `0.6666666`, `1.30` | `NaN`, `0.67`, `1.3` | `Module:fd` uses only arg 1 (and preserves the literal value, only wrapping decimals in `<small>`). `FdExpander` treats arg2 as a decimal-precision count → wrong rounding / `NaN` (real forms on Heimerdinger, Conqueror), and reparses arg1 as f64 so trailing zeros are dropped. | #20 |
| 8 | `{{times}}` | `×` | `x` | `TimesExpander` hardcodes ASCII `x`; the wiki is the multiplication sign `×`. Affects all damage/scaling `×N` text (1,248 uses). | #21 |
| 9 | `{{lethality\|10}}` | `10 Lethality (6.22 – 10 (based on level) armor penetration)` | `10 lethality` | `LethalityExpander` models only the flat number and drops the per-level armor-penetration conversion clause (`Template:Lethality` emits both). Partial-model handler — AGENTS.md flags these as more dangerous than an error. Latent: no convertible page uses `{{lethality\|N}}` directly today. | #22 |
| 10 | `{{#titleparts:A/B/C\|1}}`, `{{formatnum:12345.678}}`, `{{#ifexist:...}}` | `A`, `12,345.678`, branch | `E_UNKNOWN_TEMPLATE` | more MediaWiki magic words reached in the corpus: `#ifexist:` (77 pages), `#titleparts:` (29), `formatnum:` (6). Latent (live in unexpanded template bodies). | #23 |
| 11 | `{{#expr:0.02*0.6}}`, `{{#expr:1/3 round 10}}` | `0.012`, `0.3333333333` | `0.01`, `0.33` | `ExprExpander` calls `evaluate_expression_display(_, ctx.precision)`, rounding **every** bare `#expr` to the site precision (2 dp) — clobbering both full precision *and* an explicit `round N` (N>2), even when nested in `fd`. MediaWiki returns full precision / honours `round`. Silent precision loss; reaches real items. Violates "numbers round-trip exactly." | #24 |
| 12 | `{{ccs\|60% increased damage\|physical}}` | `60% increased damage` | `` (empty) | `CcsExpander` returns the empty string, dropping the damage-type text entirely (e.g. "deals {{ccs\|bonus damage\|physical}}" → "deals "). Content loss in ability/item text. | #25 |
| 13 | `{{#if:{{#var:undefined}}\|set\|unset}}`, `{{#ifeq:{{#expr:2+2}}\|4\|y\|n}}`, `{{#switch:{{#expr:2+3}}\|5=five\|x}}` | `unset`, `y`, `five` | `set`, `n`, `` | `#if`/`#ifeq`/`#switch` expand the selected *output branch* but NOT the condition / compared value / switch key, so a nested template there is compared as raw wikitext → wrong branch. MediaWiki expands the condition first. Low reach (conditions usually use `{{{params}}}`; `#ifeq:{{` on 2 convertible pages, `#if:{{`/`#switch:{{` on 0). | new |
| 14 | `{{#expr:2^3^2}}` | `64` | `512` | `ExprExpander` parses `^` right-associatively; MediaWiki `#expr` is left-associative (`(2^3)^2`). Latent (no chained `^` in corpus). | new |
| 15 | `{{ap\|{{#expr:80*1.4}} to {{#expr:310*1.4}} 6}}` | `112 / 176.4 / … / 434` | `` `112 to 434 6` `` (code-fenced literal) | When an `ap`/`pp` range's endpoints are themselves templates, the range isn't re-parsed after expansion — the converter fences the literal instead of interpolating. Single-value nested arithmetic (`{{ap\|(1/{{ccd\|..}})*..}}`) does work. Latent. | new |

### Round-2 sweep summary

A broader harvest (921 candidates across 28 template families) generated **593
passing self-contained tests** (`tests/wiki_examples.rs`). Families whose every
mismatch is an *intentional contract* difference (not bugs): `rd` (melee/ranged
labels), `sbc` (bold-uppercase), `tt`/`dv` (tooltip/bullet text), `gold`
(icon→word), `ft` (flip-text), `as`/`sti`/`color` (keep `'''bold'''` markup for
the markdown layer), `recurring` (combining overline `5̅` — *more* faithful than
the wiki's CSS overline, which a plain-text strip flattens to `5`). Minor, low-
priority field/format divergences noted but not separately filed: `adaptive`
leaves a `X to Y` arg as literal "to" instead of the "– (based on level)" range
on the bonus-AD side; `aug` shows the link target instead of a display-text 3rd
arg; `lll` renders `Skin (Theme)` instead of the wiki's `Theme Skin` and ignores
`text=`.

## Intentional contract differences (NOT bugs)

The converter renders a lossless *text* contract where the wiki renders
icons/colored spans/tooltips. These are by design (see `STYLE.md`) and are
locked in as characterization tests, not differential tests:

| Input | Wiki (plain-stripped) | Converter | Note |
|-------|-----------------------|-----------|------|
| `{{gold\|500}}` | `500` (+ gold icon) | `500 gold` | icon → word |
| `{{ft\|a\|b}}` | `「 a 」「 b 」` (one side shown live) | `a *(equivalently: b)*` | documented flip-text normalization |
| `{{sbc\|small caps}}` | `small caps` (CSS small-caps) | `**SMALL CAPS**` | small-caps → bold uppercase |
| `{{rd\|85\|65}}` | `( 85 / 65)` | `85 (melee) / 65 (ranged)` | melee/ranged labels made explicit |
| `{{color\|yellow\|'''Steal'''}}` | `Steal` | `'''Steal'''` | color dropped; bold markup kept for the markdown layer |
| `{{ci\|Akshan\|'s}}` | `'s` (name is icon) | `Akshan's` | name text preserved |

(`{{times}}` and `{{Divided by}}` were previously listed here but are now in the
table above: fetching their template sources showed the converter is wrong, not
making a stylistic choice.)

## Context-dependent (excluded from differential tests)

Standalone wiki renders of these differ from in-context because `ap`/`pp`
infer their level count from champion context (defaulting to ability ranks ≈6
standalone). The existing converter unit tests for the `then`-segment forms
were validated in-context by the original authors and are left untouched.

- `{{ap|<7+ explicit values>}}` — standalone wiki caps display at 6 ranks.
- `{{pp|N; then +k*x ...}}` cumulative totals and bare `then` — level count
  differs standalone vs in-context (e.g. standalone `{{pp|0; then +5*x}}`
  renders `0 – 85`, but the converter deliberately leaves a bare uncounted
  `then` raw; see the existing `pp_bare_then_without_count_is_left_raw` test).
