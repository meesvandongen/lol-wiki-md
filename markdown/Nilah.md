...
- *Formless Blade* used during *Slipstream* might not register, rendering it on cooldown without being casted.
- *Slipstream* has a chance of visually not using charge, causing it to showing one available charge without possibility of using it.
- Dashing to target through a wall within casting range will cancel the dash if wall is longer than dash range.
