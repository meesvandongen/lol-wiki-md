# Shen

## Table of Contents
- [Basic Information](#basic-information)
- [Statistics](#statistics)
  - [Map-specific Stats](#map-specific-stats)
- [Abilities](#abilities)
- [Patch History](#patch-history)
- [Trivia](#trivia)

## Basic Information

| Attribute | Value |
|-----------|------:|
| **Champion** | Shen |
| **Title** | the Eye of Twilight |
| **Resource** | Energy |
| **Range Type** | Melee |
| **Release Date** | 2010-03-24 |
| **Release Patch** | V1.0.0.81 |
| **Latest Changes** | V25.12 |
| **Roles** | Warden |
| **Riot Positions** | Top |
| **External Positions** | Top, Support |
| **Blue Essence** | 1575 |
| **Riot Points** | 790 |
| **Difficulty** | 1 |
| **Hero Type** | Tank |
| **Adaptive Type** | Physical |
| **Damage** | 2 |
| **Toughness** | 3 |
| **Control** | 2 |
| **Mobility** | 2 |
| **Utility** | 3 |
| **Style** | 45 |

## Statistics

### Base Stats

| Stat | Base | Growth |
|------|-----:|-------:|
| **Health** | $610.0$ | $+99.0$ |
| **Energy** | $400.0$ | $+0.0$ |
| **Health Regen** | $8.5$ | $+0.75$ |
| **Energy Regen** | $50.0$ | $+0.0$ |
| **Armor** | $34.0$ | $+4.2$ |
| **Magic Resist** | $32.0$ | $+2.05$ |
| **Attack Damage** | $64.0$ | $+3.0$ |
| **Attack Speed** | $0.751$ | |
| **Movement Speed** | $340.0$ | $+0.0$ |
| **Attack Range** | $125.0$ | $+0.0$ |
| **Base Attack Speed** | $0.751$ | |
| **Attack Speed Ratio** | $0.651$ | |
| **Bonus AS per Level** | $3.0\%$ | |
| **Attack Windup** | $17.4\%$ | |
| **Acquisition Radius** | $400$ units | |
| **Pathing Radius** | $35$ units | |
| **Selection Radius** | $100$ units | |
| **Selection Height** | $175$ units | |
| **Critical Damage** | $175.0\%$ | |

### Map-specific Stats

#### ARAM

| Metric | Value |
|--------|------:|
| **Damage Dealt** | $95.0\%$ |
| **Damage Taken** | $105.0\%$ |

#### URF

| Metric | Value |
|--------|------:|
| **Damage Dealt** | $100.0\%$ |
| **Damage Taken** | $105.0\%$ |

## Abilities

### Passive: Ki Barrier

| Attribute | Value |
|-----------|------:|
| **Static Cooldown** | 11 |
| **Targeting** | Passive |
| **Affects** | Self |

**INNATE:** After completing an ability's effects, **Shen** grants himself a shield for 47 to 120 (+ 13% **bonus** health) for $2.5$ seconds. If **Shen** affects at least one other champion with an ability, *Ki Barrier*’s cooldown is reduced by 4 to 8 seconds.

**Shen** manifests a Spirit Blade that he can control with *Twilight Assault* and *Spirit's Refuge*.

**Notes:**

- The Spirit Blade spawns with **Shen**, teleports closer to him if he gets too far away, and instantly blinks to him if he uses any global blink (Teleport, Recall, or Stand United). It will also dissappear if **Shen** dies and reappear upon respawning.
- Enemies see an arrow pointing from **Shen** to the location of his Spirit Blade even if it is not visible.
- Twilight Assault will grant the shield if the Spirit Blade reaches **Shen**. If it is stopped, no shield is granted. It will grant the cooldown reduction upon hitting an enemy champion.
- Spirit's Refuge will grant the shield and/or the cooldown reduction after the protective zone disappears. It does not need to block attacks to trigger the cooldown reduction, just touch an ally champion at any point of the duration.
- Shadow Dash will grant the shield when the dash ends. It will grant the cooldown reduction upon hitting an enemy champion.
  - If the dash is interrupted, the shield will instead be received the moment the crowd control source is applied, before damage calculation (if any).
- Stand United will give the shield and/or the cooldown reduction upon completing the channel.

---

### Q: Twilight Assault

| Attribute | Value |
|-----------|------:|
| **Cast Time** | none |
| **Effect Radius** | Global (Recall range) |
| **Width** | 160 (Chasing blade missile width) units |
| **Speed** | 2000 - 5000 (Acceleration 2000 per second) units/second |
| **Cost** | 140 / 130 / 120 / 110 / 100 energy |
| **Cooldown** | 8 / 7.25 / 6.5 / 5.75 / 5 seconds |
| **Cooldown Start** | on-cast |
| **Targeting** | Auto |
| **Affects** | Enemies |
| **Damage Type** | Magic |
| **Spell Shield** | True |
| **Spell Effects** | Proc |
| **Projectile** | Special |
| **Parry** | True |

**ACTIVE:** **Shen** recalls his Spirit Blade to his location. Enemy champions hit are slowed for the next 2 seconds while moving away from **Shen**.

| Attribute | Value |
|-----------|------:|
| **Slow** | 25 / 30 / 35 / 40 / 45% |

Once it arrives, **Shen** empowers his next 3 basic attacks within 8 seconds to gain range and deal **bonus** magic damage. Against structures, only the flat damage portion is applied.

| Attribute | Value |
|-----------|------:|
| **Bonus Magic Damage** | 10–40@1–16 (+ 2 / 2.5 / 3 / 3.5 / 4% (+ $1.5$% per 100 AP) of target's **maximum** health) |
| **Total Magic Damage** | 10×3 to 40×3 for 6 (+ 6 / 7.5 / 9 / 10.5 / 12% (+ $4.5$% per 100 AP) of target's **maximum** health) |

If the Spirit Blade hits at least one enemy champion during the travel, the attacks are further enhanced to deal **increased bonus** magic damage and additionally grant **Shen** as.

| Attribute | Value |
|-----------|------:|
| **Increased Bonus Damage** | 10–40@1–16 (+ 5 / 5.5 / 6 / 6.5 / 7% (+ 2% per 100 AP) of target's **maximum** health) |
| **Total Increased Damage** | 10×3 to 40×3 for 6 (+ 15 / 16.5 / 18 / 19.5 / 21% (+ 6% per 100 AP) of target's **maximum** health) |

Empowered attacks against monsters deal 100% **bonus** damage, but their **total** damage is capped.

| Attribute | Value |
|-----------|------:|
| **Maximum Monster Damage** | 120 / 140 / 160 / 180 / 200 |

**Notes:**

- The Spirit Blade will stop in place upon colliding with Wind Wall or Unbreakable. If this occurs, **Shen** will not receive the empowered attacks.
- When it is called, the Spirit Blade also checks for units which's gameplay radius it overlaps which (equal to a er 0 radius check) before flying off, allowing it to hit enemies whose center is behind the missile origin.
- The empowered attack will not trigger against wards.

---

### W: Spirit's Refuge

| Attribute | Value |
|-----------|------:|
| **Cast Time** | none |
| **Effect Radius** | 350 units |
| **Cost** | 40 energy |
| **Cooldown** | 16 / 14.5 / 13 / 11.5 / 10 seconds |
| **Cooldown Start** | post-effect |
| **Targeting** | Auto |
| **Affects** | Allies |

**ACTIVE:** **Shen** primes his Spirit Blade, creating an area around it. After 2 seconds, or when **Shen** or an allied champion enters the area, the Spirit Blade generates a protective zone for $1.75$ seconds, blocking all non-turret basic attacks that hit **Shen** or allied champions in the area.

**Notes:**

- *Spirit's Refuge*’s protective zone can be moved while active if **Shen** uses Twilight Assault.
- *Spirit's Refuge* cannot be ranked up at Level.md) 1.
- *Spirit's Refuge* will also block damage from all abilities that use basic damage (Lunge, Mystic Shot, Steel Tempest).
- Allied clones are also considered champions for this ability.

---

### E: Shadow Dash

| Attribute | Value |
|-----------|------:|
| **Cast Time** | none |
| **Target Range** | 600 (Maximum range) / 300 (Minimum range) units |
| **Collision Radius** | 60 (Dash radius) / cr 150 (Lollipop at end of dash) units |
| **Speed** | 800 + units/second |
| **Cost** | 150 energy |
| **Cooldown** | 18 / 16 / 14 / 12 / 10 (Starts after the dash ends) seconds |
| **Cooldown Start** | post-effect |
| **Targeting** | Location |
| **Affects** | Enemies |
| **Damage Type** | Physical |
| **Spell Shield** | True |
| **Spell Effects** | Area of effect |
| **Out of Range Behavior** | cast at max |
| **Grounded** | True |
| **Knockdown** | True |

**PASSIVE:** Dealing damage with *Twilight Assault* or *Shadow Dash* restores energy.

**ACTIVE:** **Shen** dashes to the target location, dealing physical damage to enemy champions and monsters he passes through and taunting them for $1.5$ seconds.

| Attribute | Value |
|-----------|------:|
| **Physical Damage** | 60 / 85 / 110 / 135 / 160 (+ 11% **bonus** health) |

*Twilight Assault and Spirit's Refuge can be cast during the dash.*

**Notes:**

- Casting Flash during *Shadow Dash* will end the dash prematurely, but the end-of-dash collision check will hit enemies at the blink's location still.
  - This still cannot affect the same enemy more than once.
- The energy restore from *Shadow Dash* may only occur once.

---

### R: Stand United

| Attribute | Value |
|-----------|------:|
| **Cast Time** | none |
| **Target Range** | Global |
| **Cooldown** | 200 / 190 / 180 / 170 / 160 seconds |
| **Cooldown Start** | on-cast |
| **Targeting** | Unit |
| **Affects** | Allies |
| **Out of Range Behavior** | N/A (range is global) |
| **Grounded** | True |
| **Silence** | True |

**ACTIVE:** **Shen** focuses his will, channeling for 3 seconds and granting the target allied champion a shield for 5 seconds at the time of cast, increased by type=target's **missing** health.

| Attribute | Value |
|-----------|------:|
| **Minimum Shield Strength** | 120 / 170 / 220 / 270 / 320 (+ 135% AP) (+ 15% of his **bonus** health) |
| **Maximum Shield Strength** | 192 / 272 / 352 / 432 / 512 (+ 216% AP) (+ 24% of his **bonus** health) |

Upon completing the channel, **Shen** and his Spirit Blade blink to the target ally's location, placed between them and the nearest enemy champion. **Shen** also destroys any projectiles targeting him at the time of completion.

**Notes:**

- Due to several character stats needing to be recalculated each time a character spawns, casting *Stand United* immediately upon respawning will cause its cooldown to be unaffected by ability haste, and the shielding will not be increased by **Shen**’s ability power, but will be increased by his **bonus** maximum health.
- *Stand United* can target allies via the mini map.md) or on their portraits or by using the **F2 - F5** keys (none of this can be done if **Shen** is affected by nearsight or his target is untargetable).
- **Shen** will turn to face towards his target ally at the start of the channel.
  - If the target is stealthed, enemies could, in theory, use this to estimate their location.
- **Shen** will channel so long as his target is alive (even if the granted shield is broken). If his target dies, the channel is canceled and *Stand United* goes on full cooldown.
  - Interrupting *Stand United*’s channel will not remove the shield prematurely.
- The target's screen will glow purple to telegraph that **Shen** has targeted them with *Stand United*.
  - It will change back when the channel ends.
- **Shen** finds the nearest visible enemy champion within 2000 range from the ally or the nearest invisible champion within 600 range to place himself 175 units from his ally towards that enemy.
- *Stand United* has a forgiveness radius of 100 units.
- The following table refers for interactions while **Shen** is channeling:

---

## Patch History

### V25.12
- Shadow Dash
  - Health ratio reduced to 11% **bonus** health from 15%.

### V25.09
- Twilight Assault
  - Empowered health ratio increased to 5 / 5.5 / 6 / 6.5 / 7% of target's **maximum** health from 4 / 4.5 / 5 / 5.5 / 6%.

### V25.08
- Twilight Assault
  - Slow increased to 25 / 30 / 35 / 40 / 45% from 15 / 20 / 25 / 30 / 35%.
- Spirit's Refuge
  - Cooldown reduced to 16 / 14.5 / 13 / 11.5 / 10 seconds from 18 / 16.5 / 15 / 13.5 / 12.

### V25.07
- Twilight Assault
  - **New Effect:** Now deals 100% of the attacks' **base** bonus damage against structures.

### V14.22
- Ki Barrier
  - Cooldown increased to 11 seconds from 10.
  - Cooldown refund increased to 4 to 8 seconds from 4 to 7.5.
- Stand United
  - Minimum base shield reduced to 120 / 220 / 320 from 130 / 290 / 450.
  - Minimum shield health ratio reduced to 15% **bonus** health from 16%.

### V14.20
- Stand United
  - **New Effect:** Cast now has a forgiveness radius of 100 units.

### V14.18
- Ki Barrier
  - Base shield increased to 47 to 120 from 47 to 101.
  - Health ratio increased to 13% **bonus** health from 12%.

### V14.6
- Ki Barrier
  - Cooldown refund increased to 4 to 7.5 seconds from 4@1; 4.5@6; 5@9; 5.5@12; 6@14; 6.5@16; 7@17; 7.5@18. *Now scales linearly instead of at breakpoints.*

### V14.2
- Stats
  - Base attack damage increased to 64 from 60.

### V13.20
- Spirit's Refuge
  - **Bug Fixes:** Now properly blocks the on-hit damage dealt by Kraken Slayer Bring It Down and Stormrazor Bolt.

## Trivia

- *Shen* is written 慎 *shèn* [ʂə̃n˥˩] "caution" in Chinese localization.
- Shen was the first champion to use energy as a resource for abilities.
- Before the release of Shen, neither him nor Zed reveal their faces in any of their skins, unlike Akali (Akali, Akali, Akali) and Kennen (Kennen,Kennen). However, Shen has never revealed his full face in any skin.
- Shen's dance references Tai chi 24-form.
  - A side-by-side comparison can be seen here.
- Shen is the first dark-themed Ionian champion, before Varus, Syndra, Zed, Jhin, Xayah, & Kayn (Noxian-born).
- Shen, Pantheon, and Twisted Fate were the first to have a global-range teleport ability.
  - After the other two's were heavily changed Shen remained the only one left with such an ability until Rek'Sai was released.
    - Rek'Sai would then later lose her global-range teleport capabilities as well.
- Shen used to gain the 'Law of Inverse Ninja Strength' cosmetic Easter egg debuff (*"This unit is a flippin' ninja!"* - *"Ninjas are more effective when they work alone. For every Ninja on your team beyond yourself, you lose 1 health."*) when he, Akali, Kennen, and/or Zed found themselves on the same team. It was removed in V3.14 for unknown reasons.

---
*This page was automatically generated from League of Legends Wiki data.*