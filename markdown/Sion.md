...
**ACTIVE:** **Sion** gains ms that decays over $2.376$ seconds.
...
- *Death Surge* will grant the bonus movement speed after a brief delay.
  - The buff is intended to grant 75% decaying movement speed, but instead grants 67% movement speed decaying with -8% per stat update.
    - Precisely this leaves him at key=%8% per stat update bonus movement speed.
...
- The **fixed** attack speed is achieved by lowering and increasing the maximum and minimum attack speed to attack speed, respectively.
  ...
  - However, Hail of Blades, which allows for the normal attack speed cap (attack speed) to be surpassed, will also break this limit. This means that while it is active, **Sion** may exceed his attack speed cap, but cannot fall below it.
    - When Hail of Blades ends, the attack speed cap will revert to attack speed and not to attack speed, however **Sion** still can't fall below attack speed.
...
- By surviving for the full duration of 60 seconds, **Sion** will have suffered key=k in health costs.
...
- At the start of *Glory in Death*, **Sion**’s mana drops to 0, but his mana regeneration will remain in effect. ;Zombie info
...
*Decimating Smash* can be recast within the duration, and does so automatically afterwards. If the charge is interrupted, *Decimating Smash* is placed on a cd cooldown.
...
If *Decimating Smash* was charged for at least 1 second, **Sion** instead slams his axe down, dealing physical damage to enemies hit, knocking them up for type=channel time seconds, and stunning them for type=channel time seconds.
...
**PASSIVE:** **Sion** gains health whenever he kills an enemy (see notes for details), increased to 15 for large enemies and takedowns against enemy champions.
...
- A buff icon in the HUD will show the current shield strength, so that the player can better calculate the timing of the second cast.
  - The initial shown value does not interact with Heal and Shield Power. Whenever the shield takes damage, it updates to the proper current shield amount, however. *The initial cast counts as an ability activation for the purposes of on-cast effects such as Spellblade and triggering Force Pulse’s passive. **Detonating the ability manually does not.
...
**ACTIVE:** **Sion** bellows a shockwave in the target direction that deals magic damage to the first enemy hit, slows them for $2.5$ seconds and inflicts them with armor penetration for 4 seconds.
...
Enemies hit by **Sion**’s charge are dealt physical damage. The *base* damage increases by type=channel time and the *scaling* increases by type=channel time.
...
Enemies, including turrets, hit by the slam are dealt the same damage and are slowed for 3 seconds. Enemies in a smaller radius (350 range, estimated) are also pulled towards **Sion** over $0.5$ seconds and become stunned after a brief delay for type=channel time seconds.
