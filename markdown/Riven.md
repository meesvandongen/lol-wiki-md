# Riven

## Table of Contents
- [Basic Information](#basic-information)
- [Statistics](#statistics)
  - [Advanced Stats](#advanced-stats)
- [Abilities](#abilities)
- [Patch History](#patch-history)
- [Trivia](#trivia)

## Basic Information

| Attribute | Value |
|-----------|-------|
| **Champion** | Riven |
| **Title** | the Exile |
| **Resource** | None |
| **Range Type** | Melee |
| **Release Date** | 2011-09-14 |
| **Release Patch** | V1.0.0.125 |
| **Roles** | Skirmisher |
| **Riot Positions** | Top |
| **External Positions** | Top |

## Statistics

### Base Stats (Level 1-18)

| Stat | Base | Growth | Level 18 |
|------|------|--------|----------|
| **Health** | $630.0$ | $+100.0$ | $2330.0$ |
| **Mana** | $0.0$ | $+0.0$ | $0.0$ |
| **Health Regen** | $8.5$ | $+0.5$ | $17.0$ |
| **Armor** | $33.0$ | $+4.4$ | $107.8$ |
| **Magic Resist** | $32.0$ | $+2.05$ | $66.8$ |
| **Attack Damage** | $64.0$ | $+3.0$ | $115.0$ |
| **Attack Speed** | $0.625$ | $+3.5\%$ | $0.997$ |
| **Movement Speed** | $340.0$ | $+0.0$ | $340.0$ |
| **Attack Range** | $125.0$ | $+0.0$ | $125.0$ |

### Advanced Stats

| Metric | Value |
|--------|-------|
| **Base Attack Speed** | $0.625$ |
| **Attack Speed Ratio** | $0.625$ |
| **Bonus AS per Level** | $3.5\%$ |
| **Acquisition Radius** | $600 units$ |
| **Pathing Radius** | $35 units$ |
| **Selection Radius** | $130 units$ |
| **Selection Height** | $150 units$ |
| **Critical Damage** | $175.0\%$ |

## Abilities

### Passive: Runic Blade

**Innate:** **Riven**’s ability casts generate stacks of *Charge*, which stacks up to a cap. ''Riven's** basic attacks consume a stack to deal **bonus'' physical damage.

**Innate:** ''Riven's* ability casts generate a stack of *Charge* for 6 seconds, refreshing on subsequent casts and stacking up to 3 times. *'Riven's** basic attacks are empowered to each consume a stack to deal **bonus'' physical damage equal to key=% AD, reduced toagainst structures. The **bonus** damage is affected by critical strike modifiers and applies lifesteal at 100% effectiveness.

| Attribute | Value |
|-----------|-------|
| **Targeting** | Passive |
| **Affects** | Enemies |
| **Damage Type** | physical |
| **Spell Effects** | Proc |

**Notes:**

- 'Runic Blade's duration also refreshes when **Riven** consumes a stack.
- The empowered attack will not trigger against wards.
- The number of empowered attacks available is represented by a counter under her health bar, only visible to the player.

---

### Q: Broken Wings

**Active:** **Riven** can cast this ability up to three times in quick succession, dealing physical damage to enemies hit. * **First Cast:** **Riven** dash forward and slashes her sword in a circle. * **Second Cast:** **Riven** mimics the first cast. * **Third Cast:** **Riven** mimics the first cast's dash, but now slams her sword in a larger circle, briefly airborne enemies hit as well.

**Active:** **Riven** can activate *Broken Wings* three times before the ability goes on cooldown, with a $0.3125$-second static cooldown between casts. If **Riven** does not recast the ability within 4 seconds of the previous cast, it goes on cooldown. **Riven** slashes with her sword for each of the three casts, dealing physical damage to enemies struck within an area, Basic attack reset her basic attack timer, and ordering her to basic attack the target of *Broken Wings* if there are any. **First Cast:** **Riven** dash 225 units in the direction she is currently facing, or up to 225 units towards the target enemy, striking enemies in the target area 100 units away. This cast cannot cross terrain. **Second Cast:** **Riven** mimics the first cast's effects. **Third Cast:** **Riven** mimics the first cast's effects in a larger area while also airborne enemies hit 75 units over $0.5$ seconds. This cast can cross terrain.

| Attribute | Value |
|-----------|-------|
| **Cooldown** | 13 seconds |
| **Cast Time** | none |
| **Targeting** | Auto / Unit |
| **Affects** | Enemies |
| **Damage Type** | physical |
| **Effect Radius** | $150$ / 250 / $200$ / 300 units |
| **Spell Shield** | True |
| **Spell Effects** | spellaoe |

**Scaling:**
- **Physical Damage:** $45-165$ bonus AD)

**Notes:**

- Each cast counts as a single ability activation for the purposes of on-cast effects such as Spellblade and triggering *Force Pulse*’s passive.
- The basic attack reset is not considered one for *Hail of Blades*.
- Moving after using *Broken Wings* allows Riven to do other actions faster.
- The **Third Cast** prevents **Riven** from using basic attacks and abilities for longer than the other 2 casts.
- The **Third Cast** may have an extended dash range when crossing terrain.

---

### W: Ki Burst

**Active:** **Riven** emits a flash of runic energy, dealing physical damage and briefly stun nearby enemies.

**Active:** **Riven** emits a flash of runic energy, dealing physical damage to nearby enemies and stun them for $0.75$ seconds.

| Attribute | Value |
|-----------|-------|
| **Cooldown** | $11-7$ seconds |
| **Cast Time** | $0.25$ seconds |
| **Targeting** | Auto |
| **Affects** | Enemies |
| **Damage Type** | physical |
| **Effect Radius** | $300 units |
| **Spell Shield** | True |
| **Spell Effects** | spellaoe |

**Scaling:**
- **Physical Damage:** $65-185$ bonus AD)

**Notes:**

- **Riven** is briefly unable to basic attack after the cast time.
- *Q* may be cast while basic attack briefly after casting *Ki Burst*.
- 'Ki Burst's effects occur before the cast time.

---

### E: Valor

**Active:** **Riven** dash in the target direction and gains a brief shield.

**Active:** **Riven** dash in the target direction, though not through terrain, while granting herself a shield for $1.5$ seconds. **Riven can cast any of her abilities during the dash. Valor will cast at max range if cast beyond that.**

| Attribute | Value |
|-----------|-------|
| **Cooldown** | $10-6$ seconds |
| **Cast Time** | none |
| **Targeting** | Direction |
| **Affects** | Self |

**Scaling:**
- **Shield Strength:** $70-170$ (+ 110% bonus AD)

**Notes:**

- *Q* may be cast during *W* or *R* cast times if they're used at the end or briefly after the dash.

---

### R: Blade of the Exile

**Active:** **Riven** empowers her blade for some time, gaining *ad **bonus** attack damage* and increased *range* on her basic attacks, **Broken Wings** and **Ki Burst**.

*During this time, **Riven** can cast **Wind Slash**.*

**Active:** **Riven** empowers her blade for 15 seconds, gaining 25% AD *ad **bonus** attack damage*, range*bonus** attack range*, and increased range on **Broken Wings** and **Ki Burst**. After $0.5$ seconds, she can cast **Wind Slash** within the duration.

| Attribute | Value |
|-----------|-------|
| **Cooldown** | $120-60$ seconds |
| **Cast Time** | $0.25$ seconds |
| **Targeting** | Auto |
| **Affects** | Self |

**Notes:**

- All bonuses are gained after $0.5$ seconds of the ability starting its cast.
  - The **bonus** attack damage amount is factored upon cast, and does not change.
- If **Riven** has a basic attack buffered before the end of 'Blade of the Exile's cast time, the buffered basic attack will start $0.33$ seconds after the end of the cast time.
  - The delay will not apply to basic attack commands input after 'Blade of the Exile's cast time.

---

### R: Wind Slash

**Active:** **Riven** unleashes a wave of energy in a cone in the target direction that deals physical damage to enemies hit based on their **missing** health.

**Active:** **Riven** unleashes a wave of energy in a cone in the target direction that deals physical damage to enemies hit, increased by type= target's **missing** health.

| Attribute | Value |
|-----------|-------|
| **Range** | 1100 units |
| **Cast Time** | $0.25$ seconds |
| **Targeting** | Direction |
| **Affects** | Enemies |
| **Damage Type** | physical |
| **Speed** | 1600 units/second |
| **Spell Shield** | True |
| **Spell Effects** | aoe |
| **Projectile** | True |

**Scaling:**
- **Minimum Physical Damage:** $100-200$ (+ 60% bonus AD)3-200×3$ (+ 180% bonus AD)

**Notes:**

- *Wind Slash* counts as an ability activation for the purposes of on-cast effects such as Spellblade and triggering *Force Pulse*’s passive. Effect at cast time end
- 'Wind Slash's wave is made up of three individual missiles spread in a cone.
  - Each enemy can be damaged by only one missile.
- If *Wind Slash* hits an enemy while *Riven* is not visible, it will briefly grant sight of the area around her to enemies.
- If **Riven** has a basic attack buffered before the end of 'Wind Slash's cast time, the buffered basic attack will start $0.528$ seconds after the end of the cast time.
  - The delay is not applied if a basic attack command is input after the cast time.

---

## Patch History

### V25.14
- *Runic Blade*
  - AD ratio reduced to key=% from key=%. *Formula is now also fully linear and no longer features a breakpoint at level 18.*
- *Broken Wings*
  - The third cast's displacement no longer prematurely ends when the affected enemy collides with a wall during it.
  - Occasionally, the third cast's airborne distance will be greatly increased.
    - This is not intended.

### V14.24
- *Valor*
  - Base shield reduced to $70-170$ from $80-180$.

### V14.20
- General
  - Recommended runes updated.
    - *Legend: Alacrity* to *Legend: Haste*.
    - *Last Stand* to *Cut Down*.
    - *Nimbus Cloak* to *Gathering Storm*.
    - *Bone Plating* to *Second Wind*.
    - *Unflinching* to *Shield Bash*.
- *Broken Wings*
  - Base damage increased to $45-165$ from $15-95$.
  - AD ratio changed to $65-85$% *bonus AD from $50-70$% **total** AD.

### V14.2
- Riven and Riven
  - *Valor*
    - **Bug Fixes:** VFX no longer unintentionally renders over impassable terrain and structures.
- Riven
  - **Bug Fixes:** VFX for her dance emote, Recall, and respawn at the base no longer become cut-off when triggered on lower graphic settings.
- Riven
  - **Bug Fixes:** No longer causes her feet to slide on the ground during her idle animations.

### V13.23
- *Broken Wings*
  - AD ratio increased to $50-70$% AD from $45-65$% AD.
- *Blade of the Exile*
  - Bonus attack damage increased to 25% AD from 20% AD.

### V13.22
- Riven
  - Skin renamed to *Reignited Worlds 2012 Riven* from *Worlds 2016 Riven*.

### V13.20
- Riven
  - Skin renamed to *Worlds 2012 Riven* from *Championship Riven*.
- Riven
  - Skin renamed to *Worlds 2016 Riven* from *Championship Riven 2016*.

### V13.4
- *Runic Blade*
  - AD ratio increased to key=% AD from key=% AD. *Now scales linearly.*
  - **New Effect:** Now applies bonus damage against structures at 50% effectiveness.
  - **New Effect:** Stacks now display on health bar for the player.

### V12.10
- Stats
  - Base health increased to 630 from 560.
  - Health growth increased to 100 from 86.
  - Armor growth increased to $4.4$ from $3.2$.
  - Magic resistance growth increased to $2.05$ from $1.25$.
- *Valor*
  - Base shield reduced to $80-180$ from $85-205$.
  - Shield bonus AD ratio reduced to 110% *bonus AD from 120%.

### V11.22
- *Ki Burst*
  - Base damage increased to $65-185$ from $55-175$.
- *Valor*
  - Bonus AD ratio increased to 120% *bonus AD from 100%.

## Trivia

- This champion has no ability power ratio.
- Riven's dance resembles the Water Dance of Braavos from Game of Thrones.
- *Runic Blade* used to be called 'Art of War' (can be seen in Riven's Champion Spotlight) while *Ki Burst* used to be called 'Ki Shout'.
- During development she was simply called *Marth* or *Exile*.
- *Riven* is the past participle of English verb *to rive* "split or tear apart violently".
  - *Riven, the Myst III: Exile* might be referencing Myst (series).

---
*This page was automatically generated from League of Legends Wiki data.*